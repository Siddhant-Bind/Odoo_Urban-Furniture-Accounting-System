...
import { ArrowLeft, Trash2, Plus, CheckCircle, FileText } from "lucide-react";
...
export default function PurchaseOrderForm() {
  ...
  const [poNumber, setPoNumber] = useState("New PO");
  ...
  useEffect(() => {
    if (!id) return;
    fetchClient(`/purchase/orders/${id}`)
      .then((po) => {
        setPoNumber(po.poNumber);
        setStatus(po.status);
        setVendorId(po.vendorId?.toString() || "");
        setOrderDate(po.orderDate ? new Date(po.orderDate).toISOString().split("T")[0] : "");
        setSavedId(po.id);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const data = await fetchClient("/purchase/orders", {
        method: "POST",
  ...
  const handleConfirm = async () => {
    ...
    setConfirming(true);
    try {
      const data = await fetchClient(`/purchase/orders/${currentId}/confirm`, { method: "POST" });
      setStatus(data.status);
    } catch (e) {
      alert(e.message);
    } finally {
  ...
  const handleCreateBill = () => {
    const currentId = savedId || parseInt(id, 10);
    navigate(`/vendor-bills/new?poId=${currentId}`);
  };
    ...
    <div className="bg-surface text-on-surface font-body-md min-h-screen flex flex-col">
      ...
      <header className="sticky top-0 z-10 bg-surface-container-lowest/95 backdrop-blur-xl border-b border-surface-container shadow-sm p-4 flex items-center justify-between gap-4 flex-wrap">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          ...
          {/* Confirmed PO: Create Bill button */}
          {status === "CONFIRMED" && (
            <button
      ...
      <main className="w-full max-w-5xl mx-auto p-4 md:p-6 lg:p-8 flex-1 flex flex-col gap-6">
        ...
        {isReadOnly && status !== "DRAFT" && (
          <div className="bg-surface-container-low rounded-xl p-4 border border-surface-container text-body-sm text-on-surface-variant">
            This Purchase Order is <span className="font-bold text-on-surface">{status}</span> and cannot be edited.
            {status === "CONFIRMED" && (
              <button
                onClick={handleCreateBill}
