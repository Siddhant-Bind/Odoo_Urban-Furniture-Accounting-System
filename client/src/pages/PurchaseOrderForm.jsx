...
export default function PurchaseOrderForm() {
  ...
  const [status, setStatus] = useState("Draft"); // Draft | Confirmed | Billed | Cancelled
  const [poNo] = useState("P00001");
      ...
      <header className="sticky top-0 z-10 bg-surface-container-lowest/90 backdrop-blur-xl border-b border-surface-container shadow-sm p-4 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            <h1 className="text-headline-sm font-bold text-on-surface">Purchase Order</h1>
            <span className="text-label-sm font-semibold text-on-surface-variant font-mono">{poNo}</span>
          </div>
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => navigate("/purchase-orders/new")} className="px-4 py-2 rounded-full font-label-md font-semibold bg-primary text-on-primary hover:bg-primary/90 transition-colors shadow-sm">New</button>
          ...
          <button onClick={() => navigate("/vendor-bills/new")} className="px-4 py-2 rounded-full font-label-md font-semibold bg-surface-container-high text-on-surface hover:bg-surface-container transition-colors shadow-sm">Create Bill</button>
      ...
      <main className="w-full max-w-5xl mx-auto p-4 md:p-6 lg:p-8 flex-1 flex flex-col gap-6">
        ...
        <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-surface-container p-6 lg:p-8 grid grid-cols-1 md:grid-cols-3 gap-6">
          <div className="flex flex-col gap-2">
            <label className="font-label-md font-bold text-on-surface">PO Number</label>
            <input readOnly value={poNo} className="w-full h-12 px-4 rounded-lg bg-surface-container-low text-on-surface-variant font-mono border border-transparent outline-none cursor-default" />
          </div>
          <div className="flex flex-col gap-2">
          ...
          <div className="flex flex-col gap-2">
            <label className="font-label-md font-bold text-on-surface">PO Date</label>
