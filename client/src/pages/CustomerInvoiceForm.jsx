...
import { ArrowLeft, Trash2, Plus, ChevronRight, FileText } from "lucide-react";
...
const COA = ["Sales Income A/c", "Bank A/c", "Cash A/c", "Other Income A/c"];

const initialItem = () => ({
  id: Date.now(),
  product: "",
  coa: "Sales Income A/c",
  analytics: "",
  qty: 1,
  unitPrice: 0,
});
...
export default function CustomerInvoiceForm() {
  ...
  const [invoiceNo] = useState("INV-2025-0001");
  ...
  const statusOptions = ["Draft", "Confirmed", "Paid", "Cancelled"];
  const statusColors = {
      ...
      <header className="sticky top-0 z-10 bg-surface-container-lowest/90 backdrop-blur-xl border-b border-surface-container shadow-sm p-4 flex items-center justify-between gap-4 flex-wrap">
        <div className="flex items-center gap-4">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-label-sm font-semibold text-on-surface-variant font-mono">{invoiceNo}</span>
          </div>
          {/* Reference chips */}
          <button onClick={() => navigate("/sales-orders/new")} className="px-3 py-1 rounded-full bg-surface-container-high text-on-surface text-label-sm font-semibold hover:bg-primary/10 hover:text-primary transition-colors inline-flex items-center gap-1">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <button onClick={() => navigate("/customer-invoices/new")} className="px-4 py-2 rounded-full font-label-md font-semibold bg-primary text-on-primary hover:bg-primary/90 transition-colors shadow-sm">New</button>
        ...
        <main className="flex-1 flex flex-col gap-6 min-w-0">
          ...
          <div className="bg-surface-container-lowest rounded-2xl shadow-sm border border-surface-container p-6 lg:p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="flex flex-col gap-2">
              ...
              <input readOnly value={invoiceNo} className="w-full h-12 px-4 rounded-lg bg-surface-container-low text-on-surface-variant font-mono border border-transparent outline-none cursor-default" />
            </div>
            <div className="flex flex-col gap-2">
              <label className="font-label-md font-bold text-on-surface">Invoice Reference</label>
              <input type="text" placeholder="e.g. PO-12345" className="w-full h-12 px-4 rounded-lg bg-surface-container-low text-on-surface border border-transparent focus:border-primary focus:ring-1 focus:ring-primary transition-all outline-none" />
            </div>
            <div className="flex flex-col gap-2">
              ...
              <select className="w-full h-12 px-4 rounded-lg bg-surface-container-low text-on-surface border border-transparent focus:border-primary focus:ring-1 focus:ring-primary transition-all outline-none">
                {CUSTOMERS.map((v) => <option key={v}>{v}</option>)}
              </select>
            </div>
            <div className="flex flex-col gap-2">
              ...
              <div className="p-4 border-t border-surface-container flex flex-col gap-3">
                ...
                <div className="flex flex-col gap-2 font-body-sm">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold text-on-surface">Debtors A/c</p>
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-semibold text-on-surface">Sales Income A/c</p>
